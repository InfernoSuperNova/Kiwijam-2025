...
using Godot;
using DebugCircle = Kiwijam2025.Debug.Meshes.DebugCircle;
using DebugLine = Kiwijam2025.Debug.Meshes.DebugLine;
using DebugMesh = Kiwijam2025.Debug.Meshes.DebugMesh;

namespace Kiwijam2025.Debug;
...
public sealed partial class DebugDraw : Node
{
    ...
    private List<DebugMesh> _debugObjects = new();
    ...
    private static void InstantiateDebugMesh(DebugMesh mesh)
    {
        Root.AddChild(mesh);
        I._debugObjects.Add(mesh);
    }
        
    #region DebugDefaults
            
    internal const float DefaultDuration = 1.0f / 60f; // TODO: Replace this with update rate from project settings
    internal const float DefaultThickness = 0f;
    internal const float DefaultRadius = 1f;
    internal const float DefaultScale = 1f;
    internal const float DefaultSpacing = 0.1f;
    internal const float DefaultCount = 10f;
    ...
    public static void Line([Optional]Vector3? start, [Optional]Vector3? end, [Optional]Color? color, float duration = DefaultDuration, float thickness = DefaultThickness, bool drawOnTop = false, DebugMesh.Type type = DebugMesh.Type.Auto )
    {
        ...
        DebugLine mesh = new DebugLine(a, b, finalColor, duration + (float)(I._lastDelta / 2), thickness, drawOnTop, type, I);
    ...
    public static void Circle([Optional]Vector3? position, [Optional]Color? color, float duration = DefaultDuration, float radius = DefaultRadius, int precision = DefaultPrecision, bool drawOnTop = false, DebugMesh.Type type = DebugMesh.Type.Auto )
    {
        ...
        DebugCircle mesh = new DebugCircle(pos, finalColor, duration, radius, precision, drawOnTop, type, I);
    ...
    #endregion
    #region DebugRaycast
    public static void Ray(PhysicsRayQueryParameters3D query, Godot.Collections.Dictionary result, float duration = DefaultDuration)
    {
    ...
    public static void Box([Optional]Vector3? cornerA, [Optional]Vector3? cornerB, [Optional] Basis? rotation, [Optional]Color? color, float duration = DefaultDuration, float thickness = DefaultThickness, bool drawOnTop = false, DebugMesh.Type type = DebugMesh.Type.Auto )
    {
        Vector3 start = cornerA ?? DefaultPos;
        Vector3 end = cornerB ?? DefaultPos;
        Vector3 center = start.Lerp(end, 0.5f);
        start -= center;
        end -= center;
    ...
    #region DebugGrid
    public static void Grid(Vector3 center, Vector3 up, Vector3 right, [Optional] Color? color, float spacing = DefaultSpacing, float countUp = DefaultCount, float countRight = DefaultCount,
        float duration = DefaultDuration, float thickness = DefaultThickness, bool drawOnTop = false,
        DebugMesh.Type type = DebugMesh.Type.Auto)
    {
        float totalSizeUp = spacing * countUp * 2;
        float totalSizeRight = spacing * countRight * 2;

        Vector3 start = center - up * totalSizeUp / 2 - right * totalSizeRight / 2;
        for (float i = 0; i < totalSizeUp; i+= spacing)
        {
            Vector3 upPos = start + up * i;
            
            Line(upPos, upPos + right * totalSizeRight, color, duration, thickness, drawOnTop, type);
        }
        for (float i = 0; i < totalSizeRight; i+= spacing)
        {
