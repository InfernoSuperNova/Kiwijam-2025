...
public static class DebugDrawNodeExtensions
{
    ...
    public static void DebugDrawLine(this Node3D node, [Optional]Vector3? end, [Optional]Color? color, float duration = DebugDraw.DefaultDuration, float thickness = DebugDraw.DefaultThickness, bool drawOnTop = false, DebugMesh.Type type = DebugMesh.Type.Auto )
    {
        ...
        /// <param name="drawOnTop">Controls whether the object draws on top of everything else.</param>
        /// <param name="type">Auto, Tangible, or Wireframe. Tangible produces a line with a radius, wireframe produces an infinitely small yet visible line. Auto is Wireframe if 0, otherwise tangible. Defaults to Auto.</param>
    public static void DebugDrawLine(this Node3D node, Node3D endNode, [Optional]Color? color, float duration = DebugDraw.DefaultDuration, float thickness = DebugDraw.DefaultThickness, bool drawOnTop = false, DebugMesh.Type type = DebugMesh.Type.Auto )
    {
